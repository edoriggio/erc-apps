let gl = require("@/demos/rasterizer/modules/gl-matrix-min")

const terrain_vertices = [];
...
function create_terrain() {
  ...
  for(let x = -0.5; x < 0.5; x = x + step) {
    for(let z = -0.5; z < 0.5; z = z + step) {
      let v1 = gl.vec3.fromValues(x, 0.0 ,z);
      let v2 = gl.vec3.fromValues(x, 0.0, z + step);
      let v3 = gl.vec3.fromValues(x + step, 0.0, z + step);
      let v4 = gl.vec3.fromValues(x + step, 0.0, z);
...
export default terrain_vertices
